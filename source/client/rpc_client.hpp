...
namespace rpc
{
    namespace client
    {
        ...
        public:
            ...
            TopicClient(bool enableDiscovery, const std::string &ip, int port)
                  ...
                  _caller(std::make_shared<rpc::client::RpcCaller>(_requestor))
            {
                ...
                if (_enableDiscovery)
                {
                    auto offline_cb = std::bind(&TopicClient::delClient, this, std::placeholders::_1);
            ...
            bool call(const std::string &method, const Json::Value &params, const RpcCaller::JsonRequestCallback &cb)
            {
                // 获取服务提供者：1. 服务发现；  2. 固定服务提供者
                BaseClient::ptr client = getClient(method);
                if (client.get() == nullptr)
        ...
        public:
                  ...
                  _topic_manager(std::make_shared<TopicManager>(_requestor))
            {
                ...
                auto msg_cb = std::bind(&TopicManager::onPublish, _topic_manager.get(),
                                        std::placeholders::_1, std::placeholders::_2);
                _dispatcher->registerHandler<TopicRequest>(MType::RSP_TOPIC, msg_cb);
                auto message_cb = std::bind(&Dispatcher::onMessage, _dispatcher.get(),
                                            std::placeholders::_1, std::placeholders::_2);
                _topic_client = ClientFactory::create(ip, port);
                _topic_client->setMessageCallback(message_cb);
                _topic_client->connect();
            }
            bool creat(const std::string &key)
            {
                return _topic_manager->creat(_topic_client->connection(), key);
            }
            bool remove(const std::string &key)
            {
                return _topic_manager->remove(_topic_client->connection(), key);
            }
            bool subscribe(const std::string &key,const TopicManager::SubCallback &cb)
            {
                return _topic_manager->subscribe(_topic_client->connection(), key,cb);
            }
            bool cancel(const std::string &key)
            {
                return _topic_manager->cancel(_topic_client->connection(), key);
            }
            bool publish(const std::string &key, const std::string &msg)
            {
                return _topic_manager->publish(_topic_client->connection(), key, msg);
            }
            void shutdown()
            {
